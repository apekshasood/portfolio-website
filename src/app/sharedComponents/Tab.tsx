...
function Tab({ label, isActive, onClick }: TabProps) {
  return (
    <div
      className={`cursor-pointer p-4 ${isActive ? 'border-solid border-l-8 outline-offset-4 border-[#C652EE]' : ''}`}
      onClick={onClick}
    >
      {label}
    </div>
...
function TabContent({ heading, yoe, description }: TabContentProps) {
    return (
      <div className="p-4">
        <h1 className='text-lg text-[#C652EE]'>{heading.toUpperCase()}</h1>
        <h1 className='text-sm py-1'>{yoe}</h1>
        <h1 className='text-sm pb-3'>Key Responsibilities:</h1>
          <ul className='list-disc ps-6 '>
            {description.map((point, index) => (
              <li key={index} className="marker:text-[#C652EE] text-sm ps-6">
              {point}
            </li>
            ))}
          </ul>
...
const tabs = [
  { id: 1, label: 'Gojoko Technology Noida', heading: 'Frontend Developer', yoe: 'Oct 2022 - Present', description: ['Design, develop, test and deploy design features in a timely manner.',
  ...
  { id: 2, label: 'Newgen Software Pvt Ltd Noida', heading: 'Sr. Software Engineer', yoe: 'Sept 2020 - Oct 2022', description: ['Made high level contributions to a team leading site optimizations.',
